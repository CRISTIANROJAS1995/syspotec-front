...
import { ScrollbarGeometry, ScrollbarPosition } from '@syspotec/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[syspotecScrollbar]',
    exportAs: 'syspotecScrollbar'
})
export class SyspotecScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_syspotecScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() syspotecScrollbar: boolean = true;
    @Input() syspotecScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'syspotecScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.syspotecScrollbar = coerceBooleanProperty(changes.syspotecScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.syspotecScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'syspotecScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.syspotecScrollbarOptions.currentValue);
