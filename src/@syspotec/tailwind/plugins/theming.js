...
// -----------------------------------------------------------------------------------------------------
// @ syspotec Technologies TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--syspotec-${e(key)}`, value], [`--syspotec-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`syspotec.customProps.background.${colorScheme}`);
            const foreground = theme(`syspotec.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--syspotec-${e(key)}`, value], [`--syspotec-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--syspotec-${e(key)}`, value], [`--syspotec-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--syspotec-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--syspotec-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                syspotec  : {
                    customProps: {
